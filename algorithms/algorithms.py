...
class Algorithms:
    ...
    def EFD_dc(self, iter_num, normalization):
        ...
        for n in range(self.num_clients):
            ...
            # print(iter_num, n, self.client_residuals[n], max(self.client_residuals[n]), min(self.client_residuals[n]))
            error_norm = torch.sum(torch.square(self.client_residuals[n])).item()
            error_mag_i.append(error_norm)

            real_update_norm = torch.sum(torch.square(Vector_update)).item()
            bt_norm = torch.sum(torch.square(Vector_update + self.client_compressor[n].discount_parameter * self.client_residuals[n])).item()

            error_to_update_ratio = (self.client_compressor[n].discount_parameter**2 * error_norm) / real_update_norm

            old_error_norm = torch.sum(torch.square(self.client_residuals[n])).item()
            old_error = self.client_residuals[n]
            # self.client_compressor[n].discount_parameter = self.gamma / (1 + old_error_norm)
            ...
            if iter_num == 0:
                new_old_to_error_ratio = 1
                self.initial_error_norm = new_error_norm
            else:
                new_old_to_error_ratio = old_error_norm / new_error_norm
            # print(new_old_to_error_ratio)
            error_to_bt = new_error_norm / bt_norm
            ...
            if self.adaptive is True:
                if self.compression_method == 'topk':
                    ...
                    # self.client_compressor[n].discount_parameter = min(self.org_gamma * learning_rate * sigma * new_old_to_error_ratio, 1)
                # print(iter_num, n, new_error_norm, normalization, self.client_compressor[n].discount_parameter)
